...
# Este modulo permite detectar conexiones de red sospechosas y consultar su reputación en VirusTotal.
...
function get-apiKey {
    if (-not $Script:ApiKey) {
        ...
        if (-not $Script:ApiKey) {
            Write-Host "Error: No se proporciono ninguna API Key. El proceso no puede continuar."
            return $null
        }
    }
...
# Funcion para consultar la reputación de una IP en VirusTotal
function consult {
    param (
        [string]$Ip    
    )
...
function detect {
    PI Key al usuario si no está disponible
    $ApiKey = Obtener-ApiKey
    ...
    foreach ($Conexion in $Conexiones) {
        ...
        # Llamamos a la funcion para consultar la reputación de la IP remota en VirusTotal
        $ResultadoVT = Consultar-VirusTotalIP -Ip $IpRemota

        # Si la consulta fue exitosa y recibimos una respuesta
        if ($ResultadoVT) {
            a IP está en su base de datos
            if ($ResultadoVT.response_code -eq 1) {
                ...
                # Si response_code es diferente de 1, significa que VirusTotal no tiene informacion sobre la IP
